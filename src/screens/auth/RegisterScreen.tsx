...
import { oryGoogleRegister, oryRegister } from '@/src/services/auth';
...
export default function RegisterScreen() {
  ...
  async function handleRegister() {
    ...
    setLoading(true);
    try {
      const result = await oryRegister(email, password);
      const email_ = result.session.identity.traits.email;
      setSessionToken(result.session_token, result.session.identity.id, email_);
      router.back();
    } catch (e) {
        ...
        >
          ...
          <View className="pt-12 pb-8">
            ...
            <Text className="text-sm text-muted-foreground mt-2 leading-5">
              Create a free account to back up your studio and sync across devices.
            </Text>
          </View>
