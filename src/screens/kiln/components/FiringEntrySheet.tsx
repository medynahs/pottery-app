import { DialogCard, DialogShell, useDialogActionHandoff } from '@/src/components/AppSheets';
...
export function FiringEntrySheet({ visible, onClose, onSchedule, onRecordPast }: FiringEntrySheetProps) {
  ...
  const handleChoice = React.useCallback(
    (action: () => void) => {
      handoff.runAfterClose(action, onClose);
    },
    [handoff, onClose],
  );
    ...
    >
      <DialogCard>
        <View className="px-6 pb-2 pt-6">
          ...
          <TouchableOpacity
            onPress={() => handleChoice(onSchedule)}
            activeOpacity={0.85}
          ...
          <TouchableOpacity
            onPress={() => handleChoice(onRecordPast)}
            activeOpacity={0.85}
