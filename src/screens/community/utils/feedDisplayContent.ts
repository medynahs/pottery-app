...
export function communityPostKindLabel(content: string): string | null {
  ...
  if (meta?.challenge?.title) return 'Challenge entry';
  if (meta) {
...
export function communityPostChallengeTitle(content: string): string | null {
  return parseCommunityPostMeta(content)?.challenge?.title ?? null;
}
