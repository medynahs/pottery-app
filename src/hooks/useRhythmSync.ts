...
import {
  fetchRhythm,
  parseStudioRhythmFromRow,
  upsertRhythm,
} from '@/src/services/rhythm';
import {
...
function latestRhythmFromRows(
  rows: Awaited<ReturnType<typeof fetchRhythm>>,
): StudioRhythm | null {
  ...
  for (const row of sorted) {
    const parsed = parseStudioRhythmFromRow(row);
    if (parsed) return normalizeStudioRhythm(parsed);
  }
  return null;
}
...
export function useRhythmSync() {
  ...
  useEffect(() => {
    ...
    const serverRhythm = latestRhythmFromRows(query.data);
