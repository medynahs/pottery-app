...
import { CHALLENGES_QUERY_KEY } from '@/src/screens/community/hooks/useChallengesQuery';
import { COMMUNITY_POLLS_QUERY_KEY } from '@/src/screens/community/hooks/useCommunityPolls';
import { FOR_YOU_FEED_QUERY_KEY } from '@/src/screens/community/hooks/useForYouFeed';
import { HALL_OF_FAME_QUERY_KEY } from '@/src/screens/community/hooks/useHallOfFameArchive';
import { PROFILE_POSTS_QUERY_KEY } from '@/src/screens/overview/profile/hooks/useProfilePosts';
...
async function invalidateStudioQueries(queryClient: ReturnType<typeof useQueryClient>) {
  await Promise.all([
    ...
    queryClient.invalidateQueries({ queryKey: FOR_YOU_FEED_QUERY_KEY }),
    queryClient.invalidateQueries({ queryKey: COMMUNITY_POLLS_QUERY_KEY }),
    queryClient.invalidateQueries({ queryKey: CHALLENGES_QUERY_KEY }),
    queryClient.invalidateQueries({ queryKey: HALL_OF_FAME_QUERY_KEY }),
    queryClient.invalidateQueries({ queryKey: PROFILE_POSTS_QUERY_KEY }),
  ]);
}
