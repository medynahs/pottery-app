...
import { isForcePremiumEnabled, resolvePremiumFromEntitlement } from '@/src/utils/forcePremium';
...
export function useEntitlementSync() {
  ...
  useEffect(() => {
    ...
    const currentPremium = useAppStore.getState().isPremium;
    setIsPremium(resolvePremiumFromEntitlement(backendPremium || currentPremium));
